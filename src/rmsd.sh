#!/bin/bash
#SBATCH -J tsspro_sde_rmsd
#SBATCH -o ../logs/rmsd.log
#SBATCH -e ../logs/rmsd.log
#SBATCH -N 1 
#SBATCH -c 4
#SBATCH -p short
#SBATCH --mem=16GB
...
# Plot name keeps the suffix last (..._rmsd_no_diffusion.png) rather than inheriting the
# trajectory's (..._no_diffusion_rmsd.png), so plots sort by metric.
PLOT=trajectories/${SYS}/${SYS}_trajectories_${STEPS}_rmsd${SUFFIX}.png

cd $SLURM_SUBMIT_DIR/..
python -u src/utils/rmsd.py ${TRAJ} --gt data/${SYS}_gt.xyz --output ${PLOT}
