#!/bin/bash
#SBATCH -J tsspro_sde_acf
#SBATCH -o ../logs/autocorr.log
#SBATCH -e ../logs/autocorr.log
#SBATCH -N 1
#SBATCH -c 4
#SBATCH -p short
#SBATCH --mem=16GB
...
# Plot name keeps the suffix last (..._acf_no_diffusion.png) rather than inheriting the
# trajectory's (..._no_diffusion_acf.png), so plots sort by metric.
PLOT=trajectories/${SYS}/${SYS}_trajectories_${STEPS}_acf${SUFFIX}.png

cd $SLURM_SUBMIT_DIR/..
python -u src/utils/autocorr.py ${TRAJ} --gt data/${SYS}_gt.xyz --output ${PLOT}
