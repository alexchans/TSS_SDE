#!/bin/bash
#SBATCH -J tsspro_sde_rama
#SBATCH -o ../logs/rama.log
#SBATCH -e ../logs/rama.log
#SBATCH -N 1 
#SBATCH -c 4
#SBATCH -p short
#SBATCH --mem=16GB
...
# Plot name keeps the suffix last (..._rama_no_diffusion.png) rather than inheriting the
# trajectory's (..._no_diffusion_rama.png), so plots sort by metric.
PLOT=trajectories/${SYS}/${SYS}_trajectories_${STEPS}_rama${SUFFIX}.png

cd $SLURM_SUBMIT_DIR/..
python -u src/utils/rama.py ${TRAJ} --gt data/${SYS}_gt.xyz --atoms data/${SYS}_atoms.txt --output ${PLOT}
